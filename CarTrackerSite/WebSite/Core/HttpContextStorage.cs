    ...
    public class HttpContextStorage
    {
        ...
        public static Car CurrentCar 
        {
            get
            {
                ...
                if (currentContext.Items["CURRENT_CAR"] == null)
                {
                    if (CookieStorage.IsCurrentCarSaved())
                    {
                        var service = DependencyResolver.Current.GetService<ICarService>();

                        currentContext.Items["CURRENT_CAR"] = service.GetById(CookieStorage.CurrentCarId.Value);
                    }
                    else
                    {
                        return null;
                ...
                return (Car)currentContext.Items["CURRENT_CAR"];
            }
        }
    }
}